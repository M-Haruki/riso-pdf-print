...
リソグラフで印刷するために必要なドライバーやツールのインストール方法を説明します。
...
::: tip
多くの場合ドライバーは、機種が異なっても使えます。  
例えば、SF625ⅡとSD5430のドライバーは、どちらもSF625ⅡとSD5430の両方で印刷可能です。
:::
...
使いたいリソグラフの型番が上記にない場合は、[プリンタードライバー・ユーティリティソフト・マニュアルのダウンロード](https://www.riso.co.jp/dl/index.html)からドライバーやツールを探すことで、同様にダウンロードできる可能性があります。
...
`インストールするドライバーを選択します`の画面では、使いたい機種`RISO SF 6x5ⅡSeries(USB)`か`RISO SD 5x30 Series(USBメモリ)`を選択して、`次へ`をクリックしてください。
![ドライバー選択](./driver-setup/install-driver.png)

`プリンタ名を入力します。`の画面では、デフォルトのまま、`次へ`をクリックしてください。

`プリンタドライバーのインストールの準備ができました。`の画面では、`インストール`をクリックしてください。
